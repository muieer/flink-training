...
public class RidesAndFaresExercise {
    ...
    /*
    * 做错了，两个流之间的连接顺序是无法控制的，所以 flatMap1 和 flatMap2 都要检查状态
    * */
    public static class EnrichmentFunction
            extends RichCoFlatMapFunction<TaxiRide, TaxiFare, RideAndFare> {

        ValueState<TaxiRide> blocked;

        @Override
        public void open(Configuration config) throws Exception {
            blocked = getRuntimeContext()
                .getState(new ValueStateDescriptor<>("blocked", TaxiRide.class));
        }
        ...
        public void flatMap2(TaxiFare fare, Collector<RideAndFare> out) throws Exception {
            if (blocked.value() != null) {
                out.collect(new RideAndFare(blocked.value(), fare));
            }
        }
    }
}
